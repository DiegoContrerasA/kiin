...
import { currencyFormat, calculateTotals } from "@/lib/mappers"
import { useWatch } from "react-hook-form"
import { PmsTypology } from "@/types/pms"


const Summary = ({ selectedRoom, isPending }: { selectedRoom: PmsTypology | null, isPending: boolean }) => {
    

    const withTransfer = useWatch({ name: 'withTransfer' })
    const withPet = useWatch({ name: 'withPet' })


    const adults = parseInt(selectedRoom?.adults || '2')
    const children = parseInt(selectedRoom?.children || '0')
            ...
            <div className="px-6 pt-6 pb-10">
                <div className="border-y border-white/20 py-6">
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-sm font-medium">{selectedRoom?.startDate}</p>
                        </div>
                        <div>
                            <p className="text-xs tracking-wide ">Check-out</p>
                            <p className="text-sm font-medium">{selectedRoom?.endDate}</p>
                        </div>
                    </div>
                </div>
                <div className="py-6 flex flex-col gap-3">
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-sm">{currencyFormat(selectedRoom?.roomPrice ?? 0)} USD</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-xs ">Pet fee</span>
                        <span className="font-medium text-sm">{currencyFormat(CONFIG.PET_FEE)} USD</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-xs ">Airport pickup</span>
                        <span className="font-medium text-sm">{currencyFormat(CONFIG.TRANSFER_FEE)} USD</span>
                    </div>

                    <div className="border-t border-white/20 py-6">
                        <div className="flex justify-between">
                            <span className="font-semibold ">TOTAL</span>
                            <span className="font-bold">{currencyFormat(total)} USD</span>
                        </div>
                    </div>
                    <div className="border-t border-white/20 py-6">
                        <div className="flex justify-between">
                            <span className="font-semibold ">DEPOSIT (50%)</span>
                            <span className="font-bold">{currencyFormat(deposit)} USD</span>
                        </div>
                    </div>
                </div>
